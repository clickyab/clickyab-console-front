...
class SessionListItem extends Component {
    render() {
        let {browser, ip, date, version, os, sessionKey, current, deleteSession} = this.props;
        return (
...
export default class SessionListPTR extends Component {
    ...
    createSessionList() {
        let {sessionList, deleteSession} = this.props,
            result = [];

        for (let i = 0; i < sessionList.sessions.length; i++) {
            result.push(<SessionListItem key={'session-' + i}
