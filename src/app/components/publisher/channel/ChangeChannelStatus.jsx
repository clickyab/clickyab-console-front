...
export default class ChangeChannelStatus extends Component {
	edit(event) {
		...
		sync(function*() {
			...
			const {response} = yield (new swagger.ChannelApi())
				.channelListAdminStatusIdPut(id, getToken(), {payloadData: {
	...
	render() {
		...
		return <select className="form-control" name="status" defaultValue={admin_status} onChange={this.edit.bind(this)}>
